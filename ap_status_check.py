...
offline_aps_file = "/opt/unifi-api-tools/offline_aps.json"
...
if login_response.status_code == 200:
    logging.info("Login successful. Starting AP-Status-Check.")
    ...
    if ap_response.status_code == 200:
        ...
        for ap in ap_list:
            ...
            if ap["state"] != 1 and ap["type"] == "uap":
                ...
                while True:
                    if time.time() > timeout:
                        ...
                        if ap_key not in offline_aps:
                            ...
                            logging.warning(f"Added {ap['name']} / {ap['mac']} to offline_aps.")
                        break
                    ...
                    if ap_status_response.status_code == 200:
                        ...
                        logging.error(
                            f"Failed to retrieve the status of access point {ap['mac']}. Moving to the next AP. "
                ...
                if ap_key in offline_aps:
                    del offline_aps[ap_key]
#                logging.info(f"Access point {ap['name']} / {ap['mac']} appears to be online.")
    ...
    if logout_response.status_code == 200:
        logging.info("Logout successful. AP-Status-Check completed.")
    else:
